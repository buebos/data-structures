...
void header() { printf("--- BOOSTACK (%zu books) ---\n\n", BOOK_LENGTH); }
...
Stack **stacks_match_one_or_create(Stack **stacks, size_t *stacks_length,
                                   Book *book, char *strategy) {
  if (book == NULL) {
    return stacks;
  }
  ...
  for (int i = 0; i < *stacks_length; i++) {
    ...
    if ((is_genre_match && strcmp(top_book->genre, book->genre) == 0) ||
...
Stack **menu_stack_by(Stack *box, size_t *stacks_length_dir, char *strategy) {
  ...
  while (box->top) {
    stacks = stacks_match_one_or_create(stacks, stacks_length_dir,
                                        box->top->data, strategy);
